...
onmessage = function(e) {
    try {
        ...
        // This catch block is for truly unexpected errors in the script itself.
...
class Solver {
    constructor(numbers, target, level) {
        ...
        // --- NEW: Timeout and Pruning Controls ---
        this.TIMEOUT_MS = 15000; // 15 seconds max calculation time
        this.startTime = 0;
        // Pruning if intermediate value is too far from target
    ...
    solve() {
        ...
        try {
            ...
            if (e.message === 'SolverTimeout') {
                console.log('Solver timed out. Returning best result found.');
            } else {
    ...
    find(items, depth) {
        ...
        if (Date.now() - this.startTime > this.TIMEOUT_MS) {
            throw new Error('SolverTimeout');
        }

        // --- NEW: Pruning based on value ---
        if(items.length < this.initialNumbers.length){
            for(const item of items){
                if(item.value > this.PRUNING_THRESHOLD && item.value > this.target) return;
            }
        }
    ...
    getBoundaryValues(items) {
        ...
        if (items.length === 2) {
            const a = items[0], b = items[1]; const results = []; let val;
