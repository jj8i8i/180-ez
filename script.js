...
function generateDerivationList(item) {
    ...
    const steps = [];
    const fullEquationStr = item.str;

    function buildSteps(currentItem, equation) {
        if (!currentItem.derivation) return;
        const inputs = currentItem.derivation.inputs;
        let nextEquation = equation;
        
        // Create the string for the current step
        const stepStr = inputs.map(inp => {
            // Replace only the first occurrence to handle duplicate numbers
            let replaced = false;
            nextEquation = nextEquation.replace(inp.str, () => {
                if (replaced) return inp.str;
                replaced = true;
                return `(${inp.derivation ? inp.derivation.resultStr : inp.value})`;
            });
            return inp.derivation ? inp.derivation.resultStr : inp.value;
        }).join(` ${currentItem.derivation.op} `);
        
        const currentStepString = `(${stepStr}) = ${currentItem.value}`;
        
        // Add step if it's new and meaningful
        if (steps.length === 0 || steps[steps.length - 1].str !== nextEquation) {
             steps.push({str: nextEquation, val: item.value});
        }

        // Recurse on inputs
        inputs.forEach(inp => buildSteps(inp, nextEquation));
    }
    
    // This is a simplified step generation, a full tree traversal is more complex
    let tempEquation = fullEquationStr;
    const queue = [item];
    const visited = new Set();
    while (queue.length > 0) {
        const node = queue.shift();
        if (!node.derivation || visited.has(node.str)) continue;
        visited.add(node.str);

        const resultValue = node.derivation.resultStr;
        const newEquation = tempEquation.replace(node.str, resultValue);
        
        if (newEquation !== tempEquation) {
            steps.push({ str: newEquation, val: item.value });
    ...
    steps.forEach(step => {
        ...
        try {
            const finalStr = `${step.str} = ${step.val}`.replace(/\*/g, '\\times');
            katex.render(finalStr, li, { throwOnError: false, displayMode: true });
        } catch (e) { li.textContent = `${step.str} = ${step.val}`; }
        list.appendChild(li);
    });
    ...
    try {
        const finalStr = `${item.str} = ${item.value}`.replace(/\*/g, '\\times');
        katex.render(finalStr, finalLi, { throwOnError: false, displayMode: true });
    } catch (e) { finalLi.textContent = `${item.str} = ${item.value}`; }
